...
public class SearchCallableKey implements Callable<ArrayList<Archive>> {
    String Key;
    Context context;
    CountDownLatch lock;
    String Account;

    public SearchCallableKey(String key, Context context, String account) {
        Key = key;
    ...
    public ArrayList<Archive> call() throws Exception {
        ...
        RequestBody requestBody = builder.setType(MultipartBody.FORM)
                .addFormDataPart("keyWord",URLEncoder.encode(Key,"UTF-8"))
                .addFormDataPart("account",Account)
                .addFormDataPart("method", "keyWord").build();
